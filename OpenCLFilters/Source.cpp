...
void parallelFilterConv(float* a) {
	cl_int error = 0;
	...
	char buffer[2048];
	error = clGetProgramBuildInfo(prog, devices[0], CL_PROGRAM_BUILD_LOG, sizeof(buffer), buffer, NULL);
	printf("%s\n", buffer);

	cl_image_format img_fmt;
	img_fmt.image_channel_order = CL_R;
	img_fmt.image_channel_data_type = CL_FLOAT;
	cl_mem image1, image3;
	size_t width = (cols + WS * 2), height = rows + WS * 2;
	image1 = clCreateImage2D(context, CL_MEM_READ_ONLY, &img_fmt, width, height, 0, 0, &error);
	err_check(error, "image1: clCreateImage2D");
	image3 = clCreateImage2D(context, CL_MEM_READ_WRITE, &img_fmt, width, height, 0, 0, &error);
	...
	cl_mem buff_filter = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float)*KERNELSIZE*KERNELSIZE, NULL, &error);

	clSetKernelArg(ker, 0, sizeof(image1), &image1);
	...
	error = clEnqueueWriteImage(com_queue, image1, CL_TRUE, origin, region, 0, 0, a, 0, NULL, NULL);
	...
	cl_event timer;
	error = clEnqueueNDRangeKernel(com_queue, ker, 2, 0, global_work_size, local_work_size, NULL, NULL, &timer);
	...
	size_t origin1[] = { WS,WS,0 }; 
	size_t region1[] = { cols, rows, 1 }; 

	vector<float> output(cols * rows);
	clEnqueueReadImage(com_queue, image3, CL_TRUE, origin1, region1, 0, 0, output.data(), NULL, NULL, NULL);


	ofstream out("blured.pgm");
	out << "P2\n" << cols << ' ' << rows << "\n255\n";
...
int main() {
	...
	for (int i = 0; i < rows + WS * 2; i++) {
		for (int j = 0; j < cols + WS * 2; j++) {
			a[cnt] = (float)file1[i][j];
			cnt++;
		}
	}

	parallelFilterConv(a);
	system("pause");
	return 0;
}